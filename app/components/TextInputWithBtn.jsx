...
const TextInputWithBtn = ({ disableBtn=false,label, id, name, type = 'text', className, placeholder, buttonChild = <></>, buttonClass='', rounded='xl', ...etc }) => {
    return (
        <div>
            <div>
                ...
                <div className="mt-0.5 flex items-center bg-white p-0.5 rounded-md">
                    <input
                        ...
                        {...etc}
                    />
                    <button disabled={disableBtn} className={`border rounded-md p-2 bg-secondary text-sm ${buttonClass}`}>
                        {buttonChild}
                    </button>
                </div>
            </div>
